...
function addMarker(place)
{
    ...
    temp.addListener('click', function() {
        ...
        $.getJSON("/articles", {geo: place.postal_code}, function(data){
            ...
            var contentString = '<div>' + '<center>' + '<h3>' + place.place_name + ", " + place.admin_name1 + ", " + place.postal_code + '</h3>' + '</center>' + '</div>';

            for(let i = 0; i < 10; i++)
            {
                contentString += '<div><ul><li>' + '<a href="' + data[i]["link"] + '">' + data[i]["title"] + '</a>' + '</li></ul></div>';
